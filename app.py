...
app.layout = html.Div([dcc.Markdown(''' # General statistics of international suicide phenomenon'''),
                       ...
                       dcc.Markdown('''The left diagram shows the absolute count of suicide deaths in each country of the dataset and the right diagram shows relative number of suicide deaths in comparison to the population of the country.'''),
                       ...
                       html.Div([
                       dcc.Dropdown(id = 'country_picker', options =options_list ,value = 'Lithuania' )],style = {'width':'20%'}, title = 'please choose a country'),
...
def update_figure(selected_year):
    ...
    bar_fig = px.bar(bar_df,x='country',y = 'suicides_no',color = 'sex',title = 'Comparison of suicide deaths of each country in {}'.format(selected_year),hover_data = ['age'])
    #Horizental bar chart
    hbar_fig = px.bar(hbar_df,x= ('proportion'),y=('country'),orientation = 'h',title = 'Ranking of highest suicide mortality per capita in {}'.format(selected_year))
...
def country_trend(selected_country):
    ...
    country_fig = px.bar(country_df,x='year',y='suicides_no',color = 'sex', title = 'follow up of the suicide deaths in {} through years'.format(selected_country), hover_data = ['age'])
